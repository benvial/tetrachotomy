...
j = sy.I
d, epsilon, mu, x, k, ky = sy.symbols("d epsilon mu x k ky")
...
for i in range(nlay + 2):
    kr = sy.sqrt(k**2 *epsilon_list[i] - ky**2)
    u.append(a_plus[i] * sy.exp(+j * kr * x) + a_minus[i] * sy.exp(-j * kr *
                                                                   x))
...
#
# htot = np.sum(h_np)
# toreplace = [(epsilon_list[0], epsilon_np[0]), (epsilon_list[-1], epsilon_np[
#     -1])]
